...
BOOL CALLBACK EnumWindowsProc(HWND hwnd, LPARAM lParam) {
    ...
    if (processedWindows.find(hwnd) == processedWindows.end()) {
        ...
        // 여기서 Windowmodule을 사용하여 창의 모서리를 그립니다.
        Windowmodule windowModule(255, 165, 0, RGB(255, 165, 0)); // 주황색으로 설정
        windowModule.AssignBorder(hwnd);
    }
    return TRUE;
}
...
static void printWindowHandles(std::unordered_set<HWND>& modifiedWindows) {
    // 창 핸들러 수집
    ...
    for (const auto& hwnd : windowHandles) {
        ...
        // Windowmodule을 사용하여 창의 모서리를 그립니다.
        Windowmodule windowModule(255, 165, 0, RGB(255, 165, 0)); // 주황색으로 설정
        windowModule.AssignBorder(hwnd);
...
int main() {
    ...
    std::thread worker([&modifiedWindows]() {
        while (true) {
            printWindowHandles(modifiedWindows);
